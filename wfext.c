...
#define lpSelA ((LPFMS_GETFILESELA)lParam)
   ...
   if (uExtSelItems == (UINT)-1) {
      ...
      if (!bSearch) {
         ...
         lstrcpy(szExtSelDirShort, szExtSelDir);

         SheShortenPath(szExtSelDirShort, TRUE);
      }
   ...
   if (bSearch) {
      ...
      if (!bLFNAware) {
         ...
         SheShortenPath(pszFileName, TRUE);
      }
...
#define lpSelA ((LPFMS_GETDRIVEINFOA)lParam)
   ...
   if (FM_GETDRIVEINFOW == uMsg)
   {
      ...
      else
      {
         ...
         wcsncpy(lpSelW->szVolume, lpszVol, COUNTOF(lpSelW->szVolume)-1);
      }
   }
   else
   {
   ...
   if (WFGetConnection(DRIVEID(szPath), &lpszVol, FALSE, ALTNAME_REG)) {
      ...
      if (FM_GETDRIVEINFOW == uMsg) {
         ...
         wcsncpy(lpSelW->szShare, lpszVol, COUNTOF(lpSelW->szShare)-1);
...
LONG
ExtensionMsgProc(UINT wMsg, WPARAM wParam, LONG lParam)
{
   HWND hwndActive;
