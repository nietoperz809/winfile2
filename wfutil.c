...
{
   INTERNALGETWINDOWTEXT InternalGetWindowText = (INTERNALGETWINDOWTEXT)GetProcAddress(GetModuleHandle (L"USER32.DLL"), "InternalGetWindowText");
   //
         ...
         if (!max_num && !num) {
            ...
            if (drive != -1) {     /* if not a search window */
               ...
               if (!dwError || DE_REGNAME == dwError) {
                  ...
                  wcsncpy(szTemp + cchTempLen, lpszVolShare,
...
ExecProgram (LPTSTR lpPath, LPTSTR lpParms, LPTSTR lpDir, BOOL bLoadIt)
{
  ...
  SetErrorMode(0);
  ret = (DWORD) RealShellExecute(hwndFrame, NULL, lpPath, lpParms, lpDir, NULL, lpszTitle, NULL, (WORD)(bLoadIt ? SW_SHOWMINNOACTIVE : SW_SHOWNORMAL), NULL);
  SetErrorMode(1);
...
DWORD ExecProgram (LPTSTR lpPath, LPTSTR lpParms, LPTSTR lpDir, BOOL bLoadIt)
{
    STARTUPINFO su;
    ...
    if (0 == CreateProcess (lpPath, lpParms, NULL, NULL, FALSE, 0, NULL, lpDir, &su, &pi))
    {   
        if (0 == CreateProcess (lpPath, lpParms, NULL, NULL, FALSE, 0, NULL, lpDir, &su, &pi))
        {
          ShellExecute (NULL, L"open", lpPath, lpParms, lpDir, SW_SHOWNORMAL);
		  return 0;
        }
    }
   ...
   register WCHAR *str)
{
   ...
   denvname[ 0 ] = CHAR_EQUAL;
   denvname[ 1 ] = CHAR_A + iDrive;
   denvname[ 2 ] = CHAR_COLON;
   denvname[ 3 ] = CHAR_NULL;

   denvvalue = SheGetEnvVar( denvname );
   if (!denvvalue) {
      *str++ = CHAR_A + iDrive;
      *str++ = CHAR_COLON;
      *str++ = CHAR_BACKSLASH;
