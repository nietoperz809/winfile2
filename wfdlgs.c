...
SaveWindows(HWND hwndMain)
{
   ...
   wsprintf(buf2, TEXT("%d,%d,%d,%d, , ,%d"), wp.rcNormalPosition.left,
...
DO_AGAIN:
   ...
   {
      GETINTERNALWINDOWPOS GetInternalWindowPos = (GETINTERNALWINDOWPOS)GetProcAddress(GetModuleHandle (L"USER32.DLL"), "GetInternalWindowPos");
...
INT
OtherDlgProc(register HWND hDlg, UINT wMsg, WPARAM wParam, LONG lParam)
{
  LONG          wView;
  ...
  switch (wMsg)
    {
      case WM_INITDIALOG:
          ...
          CheckDlgButton(hDlg, IDD_DOSNAMES, wView & VIEW_DOSNAMES);
...
INT
IncludeDlgProc(HWND hDlg, UINT wMsg, WPARAM wParam, LONG lParam)
{
  DWORD dwAttribs;
  HWND hwndActive;
...
INT
SelectDlgProc(HWND hDlg, UINT wMsg, WPARAM wParam, LONG lParam)
{
...
BOOL
FontHookProc(HWND hDlg, WORD wMsg, WPARAM wParam, LONG lParam)
{
KOREAJAPANBEGIN
...
INT
ConfirmDlgProc(HWND hDlg, UINT wMsg, WPARAM wParam, LONG lParam)
{
  UNREFERENCED_PARAMETER(lParam);
