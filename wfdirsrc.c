...
DropFilesOnApplication(LPTSTR pszFiles)
{
    ...
    cbList = 2*sizeof(*szFile) + sizeof(DROPFILESTRUCT);
    ...
    while (pszFiles = GetNextFile(pszFiles, szFile, COUNTOF(szFile))) {
       ...
       //
       // Moved SheShortenPath into wow
       //       SheShortenPath(szFile, TRUE);
       //
   ...
   BOOL bSearch)
{
   ...
   //
   // Unfortunately, we must call SheConvertPathW here since
   // RealShellExecute will only shorten the app name, not the
   // filename (it doesn't know that it's a filename; it just
   // thinks it's a parameter).
   //
   
   SheConvertPathW(szSourceFileQualified, szTemp, COUNTOF(szSourceFile));
