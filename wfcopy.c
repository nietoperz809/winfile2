...
INT ReplaceDlgProc(register HWND hDlg, UINT wMsg, WPARAM wParam, LONG lParam);
...
JAPANEND
    ...
    // Save it away.
    //
    wcsncpy(szTemp, lpszPath, COUNTOF(szTemp));
...
DiskNotThere:
   ...
   case ERROR_FILE_NOT_FOUND:
      ...
      SheChangeDir(szDrive);

      goto Retry;

   case ERROR_INVALID_PARAMETER:

      // Handle INVALID PARAMETER from SheGetDir from GetFileAttributes
...
INT
ReplaceDlgProc(register HWND hDlg, UINT wMsg, WPARAM wParam, LONG lParam)
{
   ...
   case WM_INITDIALOG:
      {
         ...
         SetWindowLong(hDlg, GWL_USERDATA, (LONG)lpdlgparams);
   ...
   BOOL *pbReadOnlyAll)
{
   ...
   if ( CONFIRMNOACCESS == dlg || CONFIRMNOACCESSDEST == dlg) {
      ...
      nRetVal = DialogBoxParam(hAppInstance, (LPTSTR) MAKEINTRESOURCE(dlg), hDlg, (DLGPROC)ReplaceDlgProc, (LONG)(LPPARAM_REPLACEDLG)&params);
...
SearchStartFail:
            ...
            if (!IsRootDirectory(pcr->sz)) {
               ...
               pcr->rgDTA->hFindFile = (HANDLE) 0xFFFFFFFF;
            }
...
IsCurrentDirectory (LPTSTR p)
{
   ...
   SheGetDirQuick(DRIVEID(p) + 1, szTemp);
#else
...
WF_CreateDirectory(HWND hwndParent, LPTSTR szDest, LPTSTR szSrc)
{
   ...
   //
   wcsncpy(szTemp, szDest, COUNTOF(szTemp)-1);
      ...
      if (pCopyInfo->dwFunc != FUNC_DELETE) {
         ...
         if (oper == OPER_DOFILE && !(bSameFile && bDoMoveRename)) {
            ...
            if (WFFindFirst(&DTADest, szDest, ATTR_ALL)) {
               ...
               lstrcpy(szShortDest, szDest);
               SheShortenPath(szShortDest, TRUE);

               lstrcpy(szShortSource, szSource);
               SheShortenPath(szShortSource, TRUE);
...
ExitLoop:
   ...
   //
   // If we left with anything hanging, tell user.
   //
   ...
   BOOL bFatalError)
{
   ...
   wcsncpy(szFile, bErrorOnDest ? pszDest : pszSource, COUNTOF(szFile));
   hDC = GetDC(NULL);
