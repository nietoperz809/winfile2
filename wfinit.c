   ...
   for (p = szBuf; *p && iNumExtensions < MAX_EXTENSIONS; p += lstrlen(p) + 1) {
      ...
      if (hMod) {
         ...
         if (fp) {
            ...
            if ((*fp)(hwndFrame, FMEVENT_LOAD, bUnicode ? (LONG)&lsW : (LONG)&lsA)) {
...
CreateSavedWindows()
{
   ...
   do {
      ...
      if (*buf) 
	  {
		 SETINTERNALWINDOWPOS SetInternalWindowPos = (SETINTERNALWINDOWPOS)GetProcAddress(GetModuleHandle(L"USER32.DLL"), "SetInternalWindowPos");
...
JAPANEND
   ...
   //
   SheGetDir(0L, szOriginalDirPath);
...
// wndClass.cbClsExtra     = 0;
   wndClass.cbWndExtra     = 2 * sizeof(LONG); // GWL_READLEVEL
// wndClass.hInstance      = hInstance;
// wndClass.hIcon          = NULL;
   wndClass.hCursor        = hcurArrow;
...
/*
   {
      ...
      hwndPrev = FindWindow (szFrameClass, NULL);
