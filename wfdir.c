...
LONG ChangeDisplay(HWND hwnd, UINT uMsg, WPARAM wParam, LONG lParam);
INT CompareDTA(register LPXDTA lpItem1, register LPXDTA lpItem2, UINT uSort);
   ...
   LONG lParam)
{
   ...
   case FS_DIRREADDONE:
   {
      ...
      if (lpStart) {
         ...
         SetWindowLong(hwnd, GWL_SELINFO, 0);
   ...
   case FS_SETSELECTION:
      ...
      //
      SendMessage(hwndLB, WM_SETREDRAW, FALSE, 0L);
      DSSetSelection(hwndLB, wParam, (LPWSTR)lParam, FALSE);
   ...
   case FS_GETSELECTION:
      ...
      // return = pszDir
      //
      return (LONG)DirGetSelection(hwnd,
                                   hwnd,
                                   hwndLB,
                                   (INT)wParam,
                                   (BOOL*)lParam,
...
#define lpci ((LPCOMPAREITEMSTRUCT)lParam)
      ...
      return (LONG)CompareDTA((LPXDTA)lpci->itemData1,
                              ...
                              (UINT)GetWindowLong(hwndParent, GWL_SORT));
#undef lpci
...
#undef lpds
   ...
   case WM_COMMAND:
      ...
      case LBN_SETFOCUS:
         ...
         //
         if (SetDirFocus(hwnd)) {
            SetWindowLong(hwndParent, GWL_LASTFOCUS, (LONG)GET_WM_COMMAND_HWND(wParam, lParam));
      ...
      case LBN_KILLFOCUS:
         ...
         SetWindowLong(hwndParent,
                       GWL_LASTFOCUS,
                       (LONG)GET_WM_COMMAND_HWND(wParam, lParam));
         break;
      }
      break;
   ...
   LONG lParam)
{
   ...
   case FS_CHANGEDISPLAY:
      ...
      case CD_VIEW:
      {
         ...
         // and Details view.
         //
         uNewView = LOWORD(lParam);
         uCurView = GetWindowLong(hwndListParms, GWL_VIEW);

         //
         // hiword lParam == TRUE means always refresh
         //
         if (uNewView == uCurView && !HIWORD(lParam))
            break;
         ...
         //
         if ((VIEW_EVERYTHING & uNewView) && (VIEW_EVERYTHING & uCurView)) {

            SetWindowLong(hwndListParms, GWL_VIEW, uNewView);
...
#ifdef PROGMAN
            if ((uNewView ^ uCurView) & VIEW_ICON) {
         ...
         //
         uNewSort = GetWindowLong(hwndListParms, GWL_SORT);
...
//DBCS dirname should not be compared ..
      ...
      if ( (WideCharToMultiByte(CP_ACP,0L,szPath,aLen,NULL,0L,NULL, NULL) == aLen) &&
       ...
       if ((wParam != CD_PATH_FORCE) && lpStart &&
         ...
         //
         uNewSort = GetWindowLong(hwndListParms, GWL_SORT);
         uNewView = GetWindowLong(hwndListParms, GWL_VIEW);
         dwNewAttribs = GetWindowLong(hwndListParms, GWL_ATTRIBS);
   ...
   case WM_CREATE:
      ...
      //
      // uNewView, uNewSort and dwNewAddribs define the viewing
      ...
      SetWindowLong(hwnd, GWL_LISTPARMS, (DWORD)hwndListParms);
      SetWindowLong(hwnd, GWL_TABARRAY, (DWORD)pwTabs);
      SetWindowLong(hwnd, GWL_SELINFO, 0);
...
CreateLB:
      ...
      if ((uNewView & VIEW_EVERYTHING) == VIEW_NAMEONLY)
      ...
      //
      // the border crap is for the non initial create case
      // I don't know why
      //
      ...
      {
         SetWindowLong(hwndListParms, GWL_VIEW, uNewView);
         SetWindowLong(hwndListParms, GWL_SORT, uNewSort);
      ...
      if (!GetWindowLong(hwndListParms, GWL_LASTFOCUS)) {
         SetWindowLong(hwndListParms, GWL_LASTFOCUS, (LONG)hwndLB);
      }
...
ResetSelection:
      ...
      if (lpStart) {
         ...
         if (pSelInfoOld != pSelInfo) {
            ...
            SetWindowLong(hwnd, GWL_SELINFO, (LONG)pSelInfo);
         }
         pSelInfo = NULL;
      }
    ...
    LPWSTR szOutStr)
{
     ...
     */
    if ( (pqSize->HighPart) &&
         ( RtlLargeIntegerToChar( pqSize,
                                  10,
                                  MAXFILENAMELEN,
                                  szBufferA ) == NO_ERROR ) )
    {
        if ( RtlMultiByteToUnicodeN( szBuffer,
                                     MAXFILENAMELEN,
                                     NULL,
                                     szBufferA,
                                     strlen(szBufferA) + 1 ) != NO_ERROR )
        {
            /*
             *  Can't seem to get the high part into string form,
             *  so just display the low part.
             *  This should never happen, but just in case.
             */
            wsprintf(szBuffer, TEXT("%lu"), pqSize->LowPart);
        }
    }
    else
    {
        /*
         *  Display just the low part.
         */
        wsprintf(szBuffer, TEXT("%lu"), pqSize->LowPart);
    }
   ...
   LPXDTALINK lpStart)
{
   ...
   //
   if ((VIEW_EVERYTHING & uViewFlags) == VIEW_NAMEONLY) {
