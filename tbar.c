...
UseRegName:
            ...
            *ppszTemp = szDrive;
            wcsncpy(szDrive+3, p, COUNTOF(szDrive)-4);
...
UnlockAndReturn:
   ...
   if (hwndExtensions && SendMessage(hwndExtensions, TB_GETBUTTON, j,
      ...
      if ((UINT)iExt < (UINT)iNumExtensions) {
         ...
         wcsncpy(lpTBNotify->pszText, tbl.szHelp, MAXDESCLEN - 1);
   ...
   case WM_NOTIFY:
       ...
       if (lpnmhdr->code == TTN_NEEDTEXT) {
           ...
           if (hwndExtensions && ((UINT)iExt < (UINT)iNumExtensions)) {
               ...
               wcsncpy(lpTT->szText, tbl.szHelp, MAXDESCLEN - 1);
