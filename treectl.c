...
UpdateSelection:
      ...
      case LBN_KILLFOCUS:
         SetWindowLong(hwndParent, GWL_LASTFOCUS, 0);
         SetWindowLong(hwndParent, GWL_LASTFOCUS, (LONG)GET_WM_COMMAND_HWND(wParam, lParam));
...
SameSelection:
      ...
      case VK_TAB:
      {
         ...
         if (bDir)
         {
            ...
            {
               SendMessage (hwndLB, LB_GETTEXT, 0, (LPARAM) &bDir);
            }
         }
         ...
         {
             SetWindowLong(hwndDir, GWL_NEXTHWND, (LONG)hwndNext);
         }
...
BuildTreeName(LPTSTR lpszPath, INT iLen, INT iSize)
{
   ...
   if (ERROR_SUCCESS == GETRETVAL(VolInfo,drive)) {
      wcsncpy(&lpszPath[iLen], aDriveInfo[drive].szFileSysName, iSize-iLen-1);
   }
