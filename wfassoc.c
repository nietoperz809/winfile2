//////////////////////////////////////////////////////////////////////////////
//
//  WFASSOC.C - Windows File Manager association code
//
//              Version 3.10
//
//              Copyright (c) 1992-1992, Microsoft Corp.  All rights reserved
//
//  FILE HISTORY:
//    AlbertT   28-Oct-1992 Created; split off from wfdlgs2.c
//    AlbertT   16-Jan-1993 Major revision in UI for compatibility
//
//////////////////////////////////////////////////////////////////////////////
...
INT
AssociateDlgProc(register HWND hDlg, UINT wMsg, WPARAM wParam, LONG lParam)
{
...
DoConfigWinIni:
      ...
      case IDD_DELETE:
         ...
         // delete an extension, we'll have a hanging extension.
...
INT
AssociateFileDlgProc(register HWND hDlg, UINT wMsg, WPARAM wParam, LONG lParam)
{
   INT i;
   DWORD dwError;
   ...
   case IDD_DDE:
      ...
      if (BN_CLICKED == GET_WM_COMMAND_CMD(wParam, lParam))
      {
         ...
         pAssociateFileDlgInfo->DDEInfo[i].bUsesDDE =
            SendDlgItemMessage(hDlg, IDD_DDE, BM_GETCHECK, 0, 0L);
   ...
   if (IDD_CONFIG != pAssociateFileDlgInfo->mode) {
      ...
      for (pExt = pFileType->pExt; pExt; pExt = pExtNext)
      {
         ...
         if (!pExt->bDelete)
         {
            ...
            SendDlgItemMessage(hDlg, IDD_EXTLIST, LB_SETITEMDATA, i, (DWORD) pExt);
         }
      }
   ...
   PASSOCIATEFILEDLGINFO pAssociateFileDlgInfo)
{
   ...
   i = (INT) SendDlgItemMessage(hDlg,IDD_EXTLIST,
      LB_ADDSTRING,0,(LONG)&pExt->szExt[1]);
      ...
      for (pft2 = pFileTypeBase; pft2; pft2 = pft2Next)
      {
         ...
         if (!lstrcmpi(pft2->lpszBuf,lpszIdent))
         {
            ...
            else
            {
               ...
               //
               wcsncpy(lpszIdent, szFileManPrefix, COUNTOF(szFileManPrefix) -1);
...
CommandWrite(HWND hDlg, LPTSTR lpszExt, LPTSTR lpszCommand)
{
   ...
   if (!*GetExtension(lpszCommand)) {
      ...
      //
      // Be careful adding .exe to {c:\foobar\joke -p}
      //
